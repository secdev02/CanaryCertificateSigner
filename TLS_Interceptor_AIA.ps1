...
Added AIA and CDP extension support for generated certificates.
...
URL for the Authority Information Access extension (CA Issuers).
Example: http://192.168.1.1:8082/i.cer

.PARAMETER CDPUrl

URL for the CRL Distribution Point extension.
Example: http://192.168.1.1:8082/i.crl
...
Interceptor.ps1 -AIAUrl "http://192.168.1.1:8082/i.cer" -CDPUrl "http://192.168.1.1:8082/i.crl"
...
# Store URLs in script scope for use in certificate creation
$script:CertAIAUrl = $AIAUrl
$script:CertCDPUrl = $CDPUrl
...
function Start-CertificateAuthority()
{
	...
	Start-Job -ScriptBlock {
			...
			While ($Hso.IsListening) {
				$HC = $Hso.GetContext()
				$HRes = $HC.Response
				$requestUrl = $HC.Request.Url.AbsolutePath
				
				if ($requestUrl -eq "/i.crl")
				{
					# Serve empty CRL (placeholder - in production you'd generate a real CRL)
					$HRes.Headers.Add("Content-Type","application/pkix-crl")
					# Return minimal CRL structure
					$HRes.StatusCode = 200
					$HRes.Close()
				}
				else
				{
					# Default: serve CA certificate
...
function Invoke-CreateCertificate([string] $certSubject, [bool] $isCA, [bool] $isWildcard)
{
    ...
    if ($isCA)
    {
        ...
        $certParams['KeyUsage'] = @("CertSign", "DigitalSignature", "CRLSign")
    ...
    else
    {
        ...
        # Add AIA extension if URL is provided
        # OID 1.3.6.1.5.5.7.1.1 = Authority Information Access
        # Format: 1.3.6.1.5.5.7.48.2 = CA Issuers access method
        if ($script:CertAIAUrl -and $script:CertAIAUrl -ne "")
        {
            $aiaExtension = "1.3.6.1.5.5.7.1.1={text}2:URL=" + $script:CertAIAUrl
            $certParams['TextExtension'] += $aiaExtension
            Write-Host ("Adding AIA extension: " + $script:CertAIAUrl) -ForegroundColor Magenta
        }
        
        # Add CDP extension if URL is provided
        # OID 2.5.29.31 = CRL Distribution Points
        if ($script:CertCDPUrl -and $script:CertCDPUrl -ne "")
        {
            $cdpExtension = "2.5.29.31={text}URI=" + $script:CertCDPUrl
            $certParams['TextExtension'] += $cdpExtension
            Write-Host ("Adding CDP extension: " + $script:CertCDPUrl) -ForegroundColor Magenta
        }
...
{	
	...
	# Display AIA/CDP configuration
	if ($script:CertAIAUrl -and $script:CertAIAUrl -ne "")
	{
		[Console]::WriteLine("AIA URL configured: " + $script:CertAIAUrl)
	}
	if ($script:CertCDPUrl -and $script:CertCDPUrl -ne "")
	{
		[Console]::WriteLine("CDP URL configured: " + $script:CertCDPUrl)
	}
